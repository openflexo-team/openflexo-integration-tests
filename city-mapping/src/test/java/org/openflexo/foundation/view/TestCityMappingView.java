...
public class TestCityMappingView extends OpenflexoProjectAtRunTimeTestCase {
	...
	public void test2LoadCityMappingViewPoint() {
		...
		FlexoBehaviour ss = cityMappingVM.getFlexoBehaviour("synchronization");
		assertNotNull(ss);
	...
	public void test6CreateVirtualModelInstance() throws SaveResourceException {
		...
		FlexoModelResource<?, ?, ?, ?> modelResource1 = project.getServiceManager().getResourceManager()
				.getModelWithURI("http://openflexo.org/city-mapping/EMF/Model/city1/my.city1");
		assertNotNull(modelResource1);
		FlexoModelResource<?, ?, ?, ?> modelResource2 = project.getServiceManager().getResourceManager()
				.getModelWithURI("http://openflexo.org/city-mapping/EMF/Model/city2/first.city2");
		...
		System.out.println("FML=" + newVirtualModelInstance.getVirtualModel().getFlexoBehaviour("synchronization").getFMLPrettyPrint());
		...
		// VirtualModelInstance.synchronize(editor) only runs a SynchronizationScheme, a deprecated
		// notion that textual FML cannot declare. The synchronization is now a plain FML method, so
		// it is executed like any other behaviour.
		ActionScheme synchronization = (ActionScheme) cityMappingVM.getFlexoBehaviour("synchronization");
		assertNotNull(synchronization);
		ActionSchemeActionFactory synchronizationType = new ActionSchemeActionFactory(synchronization, newVirtualModelInstance);
		ActionSchemeAction synchronizationAction = synchronizationType.makeNewAction(newVirtualModelInstance, null, editor);
		synchronizationAction.doAction();
		assertTrue(synchronizationAction.hasActionExecutionSucceeded());
