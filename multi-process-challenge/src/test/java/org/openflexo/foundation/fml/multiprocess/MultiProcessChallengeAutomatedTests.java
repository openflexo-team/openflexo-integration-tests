...
/**
 * A parameterized suite of FML-script driven integration tests for the MultiProcessChallenge.
 *
 * Each {@code .fmlscript} under {@code src/main/resources/AutomatedTests} is parsed and executed;
 * every {@code assert} it contains must succeed. The scripts assert business values - how many tasks
 * a process mirrors, which actor a COBOL coding task assigns itself, whether the process reaches
 * completion - not merely that the FML compiles.
 *
 * The service manager activates no technology adapter: MetaModel and AcmeMetaModel declare no model
 * slot, the whole use case being pure FML.
 *
 * @author sylvain
 */
@RunWith(Parameterized.class)
public class MultiProcessChallengeAutomatedTests extends FMLScriptParserTestCase {
	...
	public MultiProcessChallengeAutomatedTests(Resource fmlResource, String name) throws ParseException, ModelDefinitionException, IOException {
